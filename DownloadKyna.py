...
file_logger = logging.FileHandler("DownloadKyna.log", mode = 'w')
...
def Login(User, Pass):
	'''
	Name..........: Login
	Description...: Login kyna.vn
	Parameters....: User - string. User name
	..............: Pass - string. Password
	Return values.: None
	Author........: Zero-0
	'''
	url = 'https://kyna.vn/user/security/login'
	try:
		r = g_session.get(url)
	...
	authenticity_token = re.findall(r'name="_csrf"\svalue="(.*?)"', r.content)
	if not authenticity_token:
		logger.critical("Vui long lien he nha phat trien")
		return False

	payload = { 'login-form[login]' : User,
				'login-form[password]': Pass,
				'login-form[rememberMe]' : "1",
				'currentUrl' : url,
				'_csrf' : authenticity_token[0]
		}
	try:
	...
	if r.content.find(u'Sai tên đăng nhập hoặc mật khẩu') != -1:
...
def GetOnePage(url):
	...
	courses = soup.findAll('li', {'class': 'col-xl-3 col-lg-4 col-md-6 col-xs-12 k-box-card'})
	if courses == []:
	...
	for course in courses:
		content = course.find('div', {'class': 'content'})
		name = content.a.text.strip()
		if not name:
			logger.warning("Loi Phan tich tieu de khoa hoc")
			return []
		UrlCourses.append((urljoin(url, content.a['href']), NoAccentVietnamese(name).strip()))
	return UrlCourses
...
def GetCourses():
	...
	url = 'https://kyna.vn/trang-ca-nhan/khoa-hoc'
	...
	try:
		...
		lastPage = pagination.findAll('li')[-2].a.text.strip()
		UrlCourses = []
		for page in xrange(1, int(lastPage)+1):
			UrlCourse = GetOnePage('https://kyna.vn/trang-ca-nhan/khoa-hoc?page=%d' % page)
			if not UrlCourse: return []
			UrlCourses += UrlCourse
		return UrlCourses
	except Exception as e:
		logger.warning("Loi: %s - url: %s", e, url)
		return []
...
def GetLessions(url):
	...
	try:
		...
		if r.status_code != 200:
			return []
		soup = BeautifulSoup(r.content, 'html5lib')
		if soup.find('iframe', {'id': 'embed_iframe'}):
			logger.info("Khoa hoc nay khong co video bai giang. - %s" % url)
			return []
		UrlLessions = []
		details = soup.findAll('ul', {'class' : 'detail-part show'})
		for detail in details:
			lessions = detail.findAll('li', {'class': 'sub-part'})
			for lession in lessions:
				name = lession.a.text.strip()
				if not name:
					logger.warning("Loi Phan tich Ten bai giang - %s" % url)
					name = 'BG_%s' % datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d-%H-%M-%S')
...
def GetUrlMasterPlaylistFromLession(url, IsGetLinkDocument = False):
	'''
	...
	UrlMasterPlayList = re.findall(r'clip:{sources:\[{type:"application\/x\-mpegurl",src:"(.*?)"}]', r.content)
